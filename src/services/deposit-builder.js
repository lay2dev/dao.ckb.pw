import PWCore, { Builder, Cell, Transaction, RawTransaction, AmountUnit } from "@lay2/pw-core";
...
export default class DepositBuilder extends Builder {
  ...
  async build(fee) {
    ...
    let neededAmount = Builder.MIN_CHANGE.add(this.capacity)
    if (fee) neededAmount = neededAmount.add(fee)

    const inputCells = await PWCore.defaultCollector.collect(address, neededAmount)
    const inputAmount = inputCells.map(c => c.capacity).reduce((sum, a) => sum.add(a))
    ...
    const changeCell = new Cell(inputAmount.sub(neededAmount), address.toLockScript())
